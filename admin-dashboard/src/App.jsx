...
function App() {
  ...
  useEffect(() => {
    ...
    activeSocket.on("snapshot", (data) => {
      const usersObj = {};
      data.forEach(u => usersObj[u.email] = u);
      setUsers(usersObj);
    });
    ...
    <div className="min-h-screen bg-[#05080f] text-[#9ca3af] font-sans selection:bg-indigo-500/20">
      <nav className="w-full border-b border-white/[0.05] bg-[#05080f]/90 backdrop-blur-md sticky top-0 z-50 px-10 py-5 flex justify-between items-center">
        <div className="flex items-center gap-4">
          ...
          <span className="text-xl font-bold tracking-tight text-white uppercase">ActiveWatch <span className="text-indigo-500 font-medium">Command</span></span>
        </div>
        <div className="flex items-center gap-8">
          <button onClick={fetchHistory} className="text-xs font-semibold text-slate-500 hover:text-white transition-colors uppercase tracking-widest">Audit Logs</button>
          {loggedIn && (
            <button
              onClick={() => { setShowSecurity(true); setSecError(""); setSecSuccess(""); }}
              className="text-xs font-semibold text-slate-500 hover:text-indigo-400 transition-colors uppercase tracking-widest"
            >
              🔐 Security
            </button>
          )}
          <a href="http://localhost:5173" className="text-xs font-semibold text-slate-500 hover:text-white transition-colors uppercase tracking-widest">User Portal →</a>
          {loggedIn && <button onClick={logout} className="px-5 py-2 rounded-xl bg-white/[0.03] border border-white/[0.08] text-xs font-semibold hover:bg-rose-500/10 hover:text-rose-400 transition-all">TERMINATE</button>}
        </div>
      </nav>
          ...
          <div className="space-y-12 animate-in fade-in duration-500">
            <div className="flex justify-between items-end border-b border-white/[0.05] pb-10">
              <div className="space-y-2">
                <h2 className="text-5xl font-bold text-white tracking-tighter uppercase leading-none">Security <span className="text-indigo-500">Overview</span></h2>
      ...
      <footer className="w-full py-12 px-10 border-t border-white/[0.05] mt-16 text-center">
        <p className="text-[11px] font-bold text-slate-700 uppercase tracking-[0.2em]">ActiveWatch Command Center &copy; 2026</p>
      </footer>
    </div>
  );
}
